...
module variables
!Network Parameters
integer,parameter:: N_in=3,  N_bulk=120, N_out=3 
...
real(8),dimension(N_in,N_bulk):: beta_InBulk 
real(8),dimension(N_bulk,N_out):: beta_BulkOut 
real(8),dimension(N_in,N_bulk):: Rmin_InBulk 
real(8),dimension(N_bulk,N_out):: Rmin_BulkOut 
real(8),dimension(N_in,N_bulk):: Rmax_InBulk=5000
real(8),dimension(N_bulk,N_out):: Rmax_BulkOut=5000
real(8),dimension(N_in,N_bulk):: VT_ij
real(8),dimension(N_bulk,N_out):: VT_jk
...
integer:: i,j,k
real(8):: V0
real(8),parameter:: vwrite=-0.2, vread=0.00001
...
p=rand()
beta_InBulk(i,j)=1.0-p*0.2 
p=rand()
p=rand()
VT_ij(i,j)= 10.**(-1.)*(1+p)/2.d0
p=rand()
Rmin_InBulk(i,j)=100*(1+p)/2.d0
enddo;enddo

do j=1,N_bulk; do k=1,N_out
p=rand()
beta_BulkOut(j,k)=1.0-p*0.2 
p=rand()
p=rand()
VT_jk(j,k)= 10.**(-1.)*(1+p)/2.d0
p=rand()
Rmin_BulkOut(j,k)=100*(1+p)/2.d0
enddo;enddo
...
TestOutNode=po
V0=Vwrite
do step=1,5; call CircuitProcess; enddo
end subroutine
...
real(8) F !Memrsistor F function
do i=1,N_in; do j=1,N_bulk

Vm=V_in(i)-V_bulk(j)
F= -beta_InBulk(i,j)*(Vm-0.5*(abs(Vm+VT_ij(i,j))-abs(Vm-VT_ij(i,j)) ))
R_InBulk(i,j)=R_InBulk(i,j)+f*dt
...
do j=1,N_bulk; do k=1,N_out
Vm=V_bulk(j)-V_out(k)
F= -beta_BulkOut(j,k)*(Vm-0.5*(abs(Vm+VT_jk(j,k))-abs(Vm-VT_jk(j,k)) ))
R_BulkOut(j,k)=R_BulkOut(j,k)+f*dt
